...
class MPCControl_z(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        for _ in range(100):
            ...
            if np.allclose(e_next, e_bound, rtol=1e-4, atol=1e-6):
                break
            e_bound = e_next
        # Cap bounds to keep feasibility and allow faster descent
        e_bound[0] = min(e_bound[0], 0.8)
        e_bound[1] = min(e_bound[1], 0.4)
        ...
        Ke_bound = min(Ke_bound, 0.02 * (u_max - u_min))  # keep more input authority
        ...
        # 6) MPC cost/terminal
        Qv = 0.05
        Qz = 200.0  # prioritize reaching target height within ~4s
        R_mpc = np.array([[1e-4]])   # very low input penalty to allow faster descent
    ...
    def get_u(
        ...
        # Reference: reach target (delta z = 0) within 4s, then hold
        N = self.N
        k_ref = max(1, min(int(4.0 / self.Ts), N))
        z_ref = np.zeros(N + 1, dtype=float)
        z_ref[:k_ref + 1] = np.linspace(float(delta_x[1]), 0.0, k_ref + 1)
