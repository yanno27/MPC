...
class MPCControl_zvel(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
        ...
        # State is [vz] - z velocity (1D system)
        Q = np.array([[100.0]])  # Penalize z velocity heavily
        R = np.array([[1.0]])  # Input cost (average throttle)
        ...
        # Define constraints
        # Average throttle Pavg constraint: 40 <= Pavg <= 80
        # This translates to: 40 - Pavg_s <= delta_u <= 80 - Pavg_s
        # where Pavg_s = us[2] (trim throttle)
        u_min = 40.0 - self.us[0]
        u_max = 80.0 - self.us[0]
        
        # State constraints (velocity - keep loose for now)
        # No explicit constraint on vz in the problem statement
        F_x = np.array([[1.0], [-1.0]])
        f_x = np.array([20.0, 20.0])  # Loose bounds
        ...
        # Input constraints (critical for z-direction due to safety requirement)
        for k in range(N):
    ...
    def get_u(
        ...
        # Default reference is trim (zero velocity)
        if x_target is None:
