...
class MPCControl_zvel(MPCControl_base):
    """
    Part 5.1 — offset-free tracking for the z-velocity channel.

    The linear model returned by ``rocket.linearize`` is in deviation form
    (x~^+ = A x~ + B u~). All internal signals stay in deviation coordinates
    and the trim input ``self.us`` is only added when returning the command.
    A scalar disturbance state captures constant acceleration bias (e.g. mass
    mismatch).
    """

    x_ids: np.ndarray = np.array([8])   # vz in full state
    u_ids: np.ndarray = np.array([2])   # Pavg (thrust) in full input
    ...
    def _setup_controller(self) -> None:
        ...
        # Disturbance channel for reduced model: x~^+ = A x~ + B u~ + Bd d
        self.Bd = np.ones((1, 1))

        # Tuning
        self.Q = np.array([[100.0]])   # penalize vz deviation (increased for tighter tracking)
        self.R = np.array([[1.0]])     # penalize thrust deviation
        ...
        # Augmented observer for [x~; d]: z^+ = Aaug z + Baug u~
        Aaug = np.block([[self.A, self.Bd],
                         [np.zeros((1, 1)), np.ones((1, 1))]])
        Baug = np.vstack([self.B, np.zeros((1, 1))])
        Caug = np.array([[1.0, 0.0]])  # measurement y = x~

        # Place observer poles (discrete, inside unit circle)
        # Faster poles for better disturbance tracking
        p_x = 0.30  # Faster state estimation
        p_d = 0.50  # Faster disturbance estimation (was 0.70)

        a11, a12 = float(Aaug[0, 0]), float(Aaug[0, 1])
        a22 = float(Aaug[1, 1])  # = 1
        l1 = a11 + a22 - (p_x + p_d)
        if abs(a12) < 1e-12:
            raise ValueError("Bd is ~0 => disturbance not observable in this reduced model.")
        l2 = (p_x * p_d - (a11 - l1) * a22) / a12

        self.L = np.array([[l1], [l2]])

        # Observer memory (all in deviation coordinates)
        self.x_hat = np.zeros((1, 1))
        self.d_hat = np.zeros((1, 1))
        self.u_prev = np.zeros((1, 1))  # previous delta input

        # Logging for deliverable
        self.d_hat_hist = []
        self.innov_hist = []  # y - yhat
        self.d_est_history = []  # matches other axes (list of disturbance estimates)
        # Flag used by the notebook checks
        self.obs_initialized = True

    def _steady_state_input(self, r_dev: float, d_hat: float) -> tuple[float, float]:
        """
        For scalar deviation model x~^+ = A x~ + B u~ + Bd d.
        Steady-state at x~ = r_dev implies:
            r_dev = A r_dev + B u~_s + Bd d_hat -> u~_s = (r_dev - A r_dev - Bd d_hat)/B
        Returns (u_abs, u_dev).
        """
        A = float(self.A[0, 0])
        B = float(self.B[0, 0])
        ...
        if abs(B) < 1e-12:
            raise ValueError("B is ~0; cannot compute steady-state input.")
    ...
    def get_u(self, x0: np.ndarray, x_target: np.ndarray = None, u_target: np.ndarray = None):
        ...
        # ----- MPC solve (affine term uses d_hat) -----
