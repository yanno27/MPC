...
class MPCControl_xvel:
    """X-velocity MPC with simple disturbance observer (Deliverable 5.1).
    States: [omega_y, beta, vx]  (3 states for proper coupling)
    Input:  [delta2]
    """

    x_ids = np.array([1, 4, 6])  # omega_y, beta, vx
    u_ids = np.array([1])
    ...
    def __init__(self, A, B, xs, us, Ts, H):
        ...
        # Reduce system - need omega_y for proper dynamics
    ...
    def get_u(self, x0, x_target=None, u_target=None):
        # Extract X-velocity states from full state vector if needed
        ...
        if self.prob.status != cp.OPTIMAL:
            print(f"[X-vel] Warning: solver status = {self.prob.status}")
