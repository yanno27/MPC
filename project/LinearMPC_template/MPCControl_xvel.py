import numpy as np
import cvxpy as cp
from control import dlqr
from mpt4py import Polyhedron

from .MPCControl_base import MPCControl_base


"""
SWITCHABLE CONFIGURATION FOR ALL DELIVERABLES

To switch between deliverables, set the DELIVERABLE variable at the top of this file:
- DELIVERABLE = "3.1" or "3.2" : Linear simulation, hard constraints, Q(ω)=1.0
- DELIVERABLE = "4.1"           : Nonlinear simulation, soft constraints, Q(ω)=120.0

Then restart the kernel and rerun all cells.
"""

# ========== SET THIS TO SWITCH DELIVERABLES ==========
DELIVERABLE = "3"  # Change to "3" or"4"
# =====================================================


class MPCControl_xvel(MPCControl_base):
    x_ids: np.ndarray = np.array([1, 4, 6])
    u_ids: np.ndarray = np.array([1])

    def _setup_controller(self) -> None:
        #################################################
        # YOUR CODE HERE
        
        # State and input dimensions
        nx, nu = self.nx, self.nu
        N = self.N
        
        # ========== DELIVERABLE-SPECIFIC CONFIGURATION ==========
        if DELIVERABLE == "3":
            # Deliverable 3.1/3.2/3.3: Linear simulation with hard constraints
            Q_omega = 1.0
            USE_SOFT_CONSTRAINTS = False
            slack_penalty = 0.0  # Not used
            
        elif DELIVERABLE == "4":
            # Deliverable 4.1: Nonlinear simulation with soft constraints
            Q_omega = 120.0
            USE_SOFT_CONSTRAINTS = True
            slack_penalty = 4000.0

            

        
        # Build Q matrix with selected omega weight
        Q = np.diag([Q_omega, 20.0, 50.0])
        # ========================================================
        
        R = np.array([[1.0]])  # Input cost (servo deflection delta_2)
        
        # Compute LQR terminal controller and cost
        K, Qf, _ = dlqr(self.A, self.B, Q, R)
        K = -K
        A_cl = self.A + self.B @ K
        
        # Define constraints
        # Pitch angle beta constraint: |beta| <= 10 degrees = 0.1745 rad
        beta_max = 0.1745
        
        # Servo deflection delta_2: |delta_2| <= 15 degrees = 0.2618 rad
        u_min = -0.2618 - self.us[0]
        u_max = 0.2618 - self.us[0]
        
        # State constraints
        F_x = np.array([
            [0, 1, 0],   # beta <= beta_max
            [0, -1, 0],  # -beta <= beta_max
            [1, 0, 0],   # omega_y bounds (loose)
            [-1, 0, 0],
            [0, 0, 1],   # vx bounds (loose)
            [0, 0, -1]
        ])
        f_x = np.array([beta_max, beta_max, 10, 10, 20, 20])
        X = Polyhedron.from_Hrep(F_x, f_x)
        
        # Input constraints
        M_u = np.array([[1.0], [-1.0]])
        m_u = np.array([u_max, -u_min])
        U = Polyhedron.from_Hrep(M_u, m_u)
        
        # Compute terminal invariant set
        KU = Polyhedron.from_Hrep(U.A @ K, U.b)
        Xf = self._max_invariant_set(A_cl, X.intersect(KU))
        self.Xf = Xf  # Store for plotting
        
        # Define CVXPY variables
        x_var = cp.Variable((nx, N + 1))
        u_var = cp.Variable((nu, N))
        x0_param = cp.Parameter(nx)
        x_ref_param = cp.Parameter(nx)  # For reference tracking
        
        # Cost function
        cost = 0
        for k in range(N):
            cost += cp.quad_form(x_var[:, k] - x_ref_param, Q)
            cost += cp.quad_form(u_var[:, k], R)
        cost += cp.quad_form(x_var[:, N] - x_ref_param, Qf)
        
        # Constraints
        constraints = []
        
        # Initial condition
        constraints.append(x_var[:, 0] == x0_param)
        
        # System dynamics
        for k in range(N):
            constraints.append(x_var[:, k + 1] == self.A @ x_var[:, k] + self.B @ u_var[:, k])
        
        # ========== DELIVERABLE-SPECIFIC STATE CONSTRAINTS ==========
        if USE_SOFT_CONSTRAINTS:
            # Deliverable 4.1: Soft constraints with slack variables
            slack_beta = cp.Variable(N, nonneg=True)
            for k in range(N):
                constraints.append(x_var[1, k] <= beta_max + slack_beta[k])
                constraints.append(x_var[1, k] >= -beta_max - slack_beta[k])
            cost += slack_penalty * cp.sum(slack_beta)
        else:
            # Deliverable 3.1/3.2: Hard constraints
            for k in range(N):
                constraints.append(x_var[1, k] <= beta_max)
                constraints.append(x_var[1, k] >= -beta_max)
        # ===========================================================
        
        # Input constraints (always hard)
        for k in range(N):
            constraints.append(u_var[:, k] >= u_min)
            constraints.append(u_var[:, k] <= u_max)
        
        # Terminal constraint
        constraints.append(Xf.A @ x_var[:, N] <= Xf.b)
        
        # Create optimization problem
        self.ocp = cp.Problem(cp.Minimize(cost), constraints)
        self.x_var = x_var
        self.u_var = u_var
        self.x0_param = x0_param
        self.x_ref_param = x_ref_param

        # YOUR CODE HERE
        #################################################

    def _max_invariant_set(self, A_cl: np.ndarray, X: Polyhedron, max_iter: int = 30) -> Polyhedron:
        """Compute maximal invariant set for autonomous system x+ = A_cl @ x"""
        O = X
        itr = 1
        converged = False
        while itr < max_iter:
            Oprev = O
            F, f = O.A, O.b
            O = Polyhedron.from_Hrep(np.vstack((F, F @ A_cl)), np.concatenate((f, f)))
            O.minHrep(True)
            _ = O.Vrep
            if O == Oprev:
                converged = True
                break
            itr += 1
        
        if not converged:
            print(f"Warning: Terminal set did not converge after {max_iter} iterations")
        
        return O

    def get_u(
        self, x0: np.ndarray, x_target: np.ndarray = None, u_target: np.ndarray = None
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        #################################################
        # YOUR CODE HERE
        
        if x0.shape[0] > self.nx:
            x0 = x0[self.x_ids]
        
        if x_target is not None and x_target.shape[0] > self.nx:
            x_target = x_target[self.x_ids]
        
        if x_target is None:
            x_ref = self.xs
        else:
            x_ref = x_target
        
        delta_x0 = x0 - self.xs
        delta_x_ref = x_ref - self.xs
        
        self.x0_param.value = delta_x0
        self.x_ref_param.value = delta_x_ref
        
        self.ocp.solve(solver=cp.CLARABEL, verbose=False)
        
        if self.ocp.status != cp.OPTIMAL:
            print(f"Warning: Optimization problem status is {self.ocp.status}")
            u0 = np.zeros(self.nu)
            x_traj = np.tile(x0.reshape(-1, 1), (1, self.N + 1))
            u_traj = np.zeros((self.nu, self.N))
            return u0, x_traj, u_traj
        
        delta_u_opt = self.u_var.value
        delta_x_opt = self.x_var.value
        
        u0 = delta_u_opt[:, 0] + self.us
        x_traj = delta_x_opt + self.xs.reshape(-1, 1)
        u_traj = delta_u_opt + self.us.reshape(-1, 1)

        # YOUR CODE HERE
        #################################################

        return u0, x_traj, u_traj