...
class MPCControl_xvel(MPCControl_base):
    """X-velocity MPC controller with integral action (Deliverable 5)"""
    ...
    def _setup_controller(self) -> None:
        ...
        # Different gains for different states [ωy, β, vx]
    ...
    def get_u(self, x0, x_target=None, u_target=None):
        ...
        if self.ocp.status != cp.OPTIMAL:
            print(f"Warning: X-vel optimization problem status is {self.ocp.status}")
            u0 = np.zeros(self.nu)
